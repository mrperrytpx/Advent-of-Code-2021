...
function fieldSize(field, row, col, visited = new Set()) {
    ...
    const valid = [above, below, leftOf, rightOf].filter((elem) => elem !== '9'
        ...
        && field[elem[0]][elem[1]] - value == 1
        && field[elem[0]][elem[1]] != '9'
    );
    // console.log("Currently checking ", row, col, ", Valid is: ", valid);
...
// console.log(fieldSize(file, 4, 6));
// console.log(fieldSize(file, 0, 1));
// console.log(fieldSize(file, 2, 2));
// console.log(fieldSize(file, 0, 9));
// console.log("Sizes of all areas: ", sizes);
sizes = sizes.sort((a, b) => b - a).slice(0, 3).reduce((a, b) => a * b, 1);

// console.log("Three biggest areas multiplied: ", sizes);
console.log(sizes);